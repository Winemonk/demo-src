    ...
    public class GraphForceDirectedAlgorithm
    {
        ...
        private void CalculateRepulsive()
        {
            ...
            for (int v = 0; v < mNodeList.Count; v++)
            {
                ...
                mDxMap.Add(pStartShapeCircle, 0.0);
                mDyMap.Add(pStartShapeCircle, 0.0);
                for (int u = 0; u < mNodeList.Count; u++)
                {
                    if (u != v)
                    {
                        ...
                        //if (dist >= 0 && dist <= 200)
                        //{
                        //    if (mDxMap.ContainsKey(pStartShapeCircle))
                        //        mDxMap[pStartShapeCircle] = mDxMap[pStartShapeCircle] + (distX / dist * k * k / dist);
                        //    if (mDyMap.ContainsKey(pStartShapeCircle))
                        //        mDyMap[pStartShapeCircle] = mDyMap[pStartShapeCircle] + (distY / dist * k * k / dist);
                        //}

                        if (dist < (pStartShapeCircle.Width / 2 + pEndShapeCircle.Width / 2)*1.5)
                        {
                            if (mDxMap.ContainsKey(pStartShapeCircle))
                                mDxMap[pStartShapeCircle] = mDxMap[pStartShapeCircle] + (distX / dist * k * k / dist) * 3;
                            if (mDyMap.ContainsKey(pStartShapeCircle))
                                mDyMap[pStartShapeCircle] = mDyMap[pStartShapeCircle] + (distY / dist * k * k / dist) * 3;
                        }
                        else if (dist <= 200)
                        {
                            if (mDxMap.ContainsKey(pStartShapeCircle))
        ...
        private void CalculateTraction()
        {
            ...
            for (int e = 0; e < mEdgeList.Count; e++)
            {
                ...
                if (mDxMap.ContainsKey(eStartID))
                    mDxMap[eStartID] = mDxMap[eStartID] - (distX * dist * dist / k);
                else
                    mDxMap.Add(eStartID, mDxMap[eStartID] - (distX * dist * dist / k));
                if (mDyMap.ContainsKey(eStartID))
                    mDyMap[eStartID] = mDyMap[eStartID] - (distY * dist * dist / k);
                else
                    mDyMap.Add(eStartID, mDyMap[eStartID] - (distY * dist * dist / k));

                if (mDxMap.ContainsKey(eEndID))
                    mDxMap[eEndID] = mDxMap[eEndID] + (distX * dist * dist / k);
                else
                    mDxMap.Add(eEndID, mDxMap[eEndID] + (distX * dist * dist / k));
                if (mDyMap.ContainsKey(eEndID))
                    mDyMap[eEndID] = mDyMap[eEndID] + (distY * dist * dist / k);
                else
                    mDyMap.Add(eEndID, mDyMap[eEndID] + (distY * dist * dist / k));
            }
        }
        ...
        {
            int maxt = 1, maxty = 1; //Additional coefficients.
            for (int v = 0; v < mNodeList.Count; v++)
            {
